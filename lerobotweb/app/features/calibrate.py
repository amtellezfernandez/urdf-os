...
class CalibrationStatus:
    ...
    status: str = (
        "idle"  # "idle", "connecting", "homing", "recording", "completed", "error", "stopping"
    )
    ...
    recorded_ranges: Dict[str, Dict[str, float]] = (
        None  # {motor: {min: val, max: val, current: val}}
    )
...
class CalibrationManager:
            ...
            if (
                ...
                try:
                    ...
                    for attempt in range(2):  # Quick retry for status updates
                        try:
                            positions = self.device.bus.sync_read(
    ...
    def stop_calibration_process(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.calibration_thread and self.calibration_thread.is_alive():
                logger.warning(
                    "Calibration thread did not finish within timeout, forcing cleanup"
                )
    ...
    def _calibration_worker(self, request: CalibrationRequest):
        ...
        try:
            ...
            self._cleanup_and_finish(
                "Calibration completed successfully", status="completed"
            )
    ...
    def _step_homing(self):
        ...
        actual_positions = self.device.bus.sync_read(
            "Present_Position", normalize=False
        )
    ...
    def _step_range_recording(self):
        ...
        for attempt in range(5):  # Try multiple times to get valid initial positions
            try:
                ...
                if len(valid_positions) == len(positions):  # All positions are valid
                    ...
                    logger.warning(
                        f"Attempt {attempt + 1}: Got invalid initial positions, retrying..."
                    )
                    time.sleep(0.1)
            except Exception as e:
                logger.warning(
                    f"Attempt {attempt + 1}: Failed to read initial positions: {e}"
                )
                time.sleep(0.1)

        if not self._start_positions:
            raise RuntimeError(
                "Could not get valid initial positions after multiple attempts"
            )
            ...
            try:
                ...
                if positions:
                    ...
                    for motor, pos in positions.items():
                        ...
                        if (
                            pos > 0 and pos < 5000
                        ):  # Reasonable range for motor positions
                            valid_positions[motor] = pos
                        else:
                            logger.debug(
                                f"Filtered invalid position for {motor}: {pos}"
                            )
        ...
        same_min_max = [
            motor
            for motor in self._mins.keys()
            if self._mins[motor] == self._maxes[motor]
        ]
        if same_min_max:
            raise ValueError(
                f"Some motors have the same min and max values: {same_min_max}"
            )
        ...
        if insufficient_range:
            ...
            logger.warning(
                "Consider moving all joints through their full range of motion during calibration"
            )

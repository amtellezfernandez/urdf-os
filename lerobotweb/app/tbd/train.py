...
class TrainingRequest(BaseModel):
    ...
    policy_type: str = (
        "act"  # --policy.type (act, diffusion, pi0, smolvla, tdmpc, vqbet, pi0fast, sac, reward_classifier)
    )
    ...
    env_type: Optional[str] = (
        None  # --env.type (aloha, pusht, xarm, gym_manipulator, hil)
    )
    ...
    optimizer_type: Optional[str] = (
        "adam"  # --optimizer.type (adam, adamw, sgd, multi_adam)
    )
    optimizer_lr: Optional[float] = (
        None  # --optimizer.lr (will use policy default if not set)
    )
...
class TrainingManager:
        ...
        try:
            if self.process and self.process.poll() is None:
                ...
                try:
                    ...
                    logger.warning(
                        "Training process didn't terminate gracefully, forcing kill"
                    )
                    self.process.kill()
                    self.process.wait()
    ...
    def _build_training_command(self, request: TrainingRequest) -> list:
        ...
        if request.dataset_episodes:
            cmd.extend(
                ["--dataset.episodes"] + [str(ep) for ep in request.dataset_episodes]
            )
        ...
        if request.optimizer_weight_decay is not None:
            cmd.extend(
                ["--optimizer.weight_decay", str(request.optimizer_weight_decay)]
            )
        if request.optimizer_grad_clip_norm is not None:
            cmd.extend(
                ["--optimizer.grad_clip_norm", str(request.optimizer_grad_clip_norm)]
            )
        ...
        try:
            for line in iter(self.process.stdout.readline, ""):
                ...
                if line.strip():
                    ...
                    self.log_queue.put(
                        {"timestamp": time.time(), "message": line.strip()}
                    )

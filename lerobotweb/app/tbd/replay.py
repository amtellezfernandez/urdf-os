...
def run_replay_directly(request: ReplayRequest):
    ...
    try:
        ...
        logger.info(
            f"📁 Dataset: {request.dataset_repo_id}, Episode: {request.episode}"
        )
        ...
        try:
            dataset_config = DatasetReplayConfig(
                repo_id=request.dataset_repo_id, episode=request.episode, fps=30
            )
        ...
        try:
            ...
            logger.info(
                f"🔍 Robot config - Port: {robot_config.port}, ID: {robot_config.id}"
            )
            ...
            if hasattr(test_robot, "bus") and hasattr(test_robot.bus, "models"):
                if not test_robot.bus.models:
                    ...
                    raise ValueError(
                        "No motors detected on robot bus. Check robot connection and calibration."
                    )
                else:
                    logger.info(
                        f"✅ Robot has {len(test_robot.bus.models)} motors configured"
                    )
            ...
            if (
                ...
                raise ValueError(
                    f"Robot port '{request.robot_port}' is not accessible. Check USB connection and permissions."
                )
        ...
        try:
            ...
            raise ValueError(
                "No motors detected during replay. The robot may have been disconnected or the motors are not properly configured. Please check your robot hardware setup and calibration files."
            )
        except Exception as replay_error:
            logger.error(f"❌ Lerobot replay function failed: {replay_error}")
            raise

        replay_status.update({"status": "completed", "replay_active": False})
        ...
        replay_status.update(
            {"status": "error", "replay_active": False, "error_message": error_msg}
        )
    finally:
        replay_active = False
...
def handle_start_replay(request: ReplayRequest) -> Dict[str, Any]:
    ...
    if replay_active:
        return {"success": False, "message": "Replay is already active"}

    try:
        replay_active = True
        replay_thread = threading.Thread(
            target=run_replay_directly, args=(request,), daemon=True
        )
        replay_thread.start()
...
def handle_stop_replay() -> Dict[str, Any]:
    ...
    replay_active = False
    replay_status.update({"replay_active": False, "status": "stopped"})
...
def handle_replay_status() -> Dict[str, Any]:
    ...
    return {"success": True, "status": replay_status.copy()}


def handle_replay_logs() -> Dict[str, Any]:
    """Handle getting recent replay logs"""
    return {"success": True, "logs": []}  # Logs are handled by lerobot's logging system
